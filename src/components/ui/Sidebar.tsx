...
const Sidebar: React.FC<SidebarProps> = ({ role, isOpen, onClose }) => {
  ...
  // Sync open menus saat location.pathname berubah
  useEffect(() => {
      ...
      return (
        <li key={item.label}>
          <button
            ...
            className={`flex w-full items-center rounded-lg p-3 text-base font-medium transition-colors ${
              isChildActive ? 'bg-white/10 text-white' : 'text-white hover:bg-white/10 hover:text-white'
            }`}
    ...
    return (
      <li key={item.label}>
        <NavLink
          to={item.path!}
          className={({ isActive }) =>
            `flex items-center rounded-lg p-3 text-base font-semibold transition-colors ${
              isActive ? 'bg-white/20 text-white pulse-glow' : 'text-white hover:bg-white/10'
            }`
          }
        >
          <Icon className="h-6 w-6" />
