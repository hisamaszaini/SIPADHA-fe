...
import { updateProfileSchema, type UpdateProfileDto } from '../types/user.types';
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="relative mb-4">
                <div className="w-28 h-28 bg-gradient-to-br from-blue-400 to-emerald-500 rounded-full flex items-center justify-center text-white text-5xl font-bold">
                  {(user?.penduduk?.nama || user?.username)?.charAt(0).toUpperCase()}
                </div>
                <span className="absolute bottom-1 right-1 block h-5 w-5 rounded-full bg-green-400 border-2 border-white"></span>
              </div>

              {/* Nama dan Role */}
              <h2 className="text-2xl font-bold text-gray-800 capitalize">{user?.penduduk?.nama || user?.username}</h2>
            ...
            <div className="space-y-4 text-gray-600">
              {user?.penduduk?.nik && (
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-mono">{user?.penduduk?.nik}</p>
                  </div>
                </div>)}
              <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center space-x-3">
                <Icon className="fas fa-phone w-5 text-center" />
                <div className="flex-1">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
              ...
              <div className="flex justify-end mt-4">
                <Button
                  ...
                  size="mid"
                  isLoading={isPending}
                  icon="fas fa-save" // Tambahkan ikon simpan
                >
                  Simpan Perubahan
                </Button>
              </div>
